...
class StrategyCompiler:
    ...
    def load_spec(self, spec_path: Path) -> StrategySpec:
        ...
        # Preserve any embedded guided-builder UI metadata (x_guided_ui) so
        # editors and downstream tools can round-trip raw rule objects.
        x_meta = None
        if isinstance(data, dict) and "x_guided_ui" in data:
            x_meta = data.get("x_guided_ui")
            # Validate model against the spec portion only (exclude UI meta)
            spec_data = dict(data)
            spec_data.pop("x_guided_ui", None)
        else:
            spec_data = data

        spec = StrategySpec.model_validate(spec_data)
        # Attach metadata to the returned model for editors to consume.
        try:
            # For pydantic BaseModel instances attaching arbitrary attrs may
            # not be visible via setattr in some configs; write directly into
            # the instance __dict__ as a best-effort approach.
            spec.__dict__["x_guided_ui"] = x_meta
        except Exception:
            try:
                setattr(spec, "x_guided_ui", x_meta)
            except Exception:
                pass
        return spec
