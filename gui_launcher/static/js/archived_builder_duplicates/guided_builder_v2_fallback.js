/* ARCHIVE: minimal fallback harness for guided builder (moved to archive)
   Preserved to allow later reference.
*/
(function () {
  ...
  function serialize(container) {
    ...
    for (const r of container.querySelectorAll('.rule-row')) {
      ...
      const sideEl = r.querySelector('[data-side]'); if (sideEl && sideEl.value) rule.side = String(sideEl.value).trim().toLowerCase();
      out.push(rule);
    }
    return out;
  }
  ...
  function createRow(section, label) {
    ...
    const side = el('select'); side.setAttribute('data-side',''); side.appendChild(el('option',{value:'both',text:'both'})); side.appendChild(el('option',{value:'long',text:'long'})); side.appendChild(el('option',{value:'short',text:'short'}));
